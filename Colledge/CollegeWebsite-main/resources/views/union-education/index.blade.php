...
@section('title', \App\Models\PageSection::getValue('union_education', 'metadata', 'title', 'Профсоюзное обучение'))
    ...
    <div class="container mx-auto px-4">
        ...
        <div class="text-center mb-12">
            <h1 class="text-3xl md:text-4xl font-bold text-white mb-4">
                {{ \App\Models\PageSection::getValue('union_education', 'hero', 'main_title', 'Профсоюзное обучение') }}
            </h1>
            <div class="w-24 h-1 bg-gradient-to-r from-blue-500 to-cyan-500 mx-auto mb-6"></div>
            <p class="text-gray-300 text-lg max-w-3xl mx-auto">
                {{ \App\Models\PageSection::getValue('union_education', 'hero', 'main_description', 'Нормативные документы и учебные материалы для профсоюзных активистов. Все документы представлены в формате PDF для просмотра и скачивания.') }}
            </p>
        </div>
                ...
                <div class="sticky top-24">
                    <div class="bg-gradient-to-r from-blue-900/30 via-blue-800/30 to-cyan-900/30 rounded-xl p-6 mb-6 border border-blue-500/20">
                        <h2 class="text-xl font-bold text-white mb-4">
                            {{ \App\Models\PageSection::getValue('union_education', 'navigation', 'documents_title', 'Нормативные документы') }}
                        </h2>
                        <p class="text-gray-300 text-sm">{{ \App\Models\PageSection::getValue('union_education', 'navigation', 'documents_subtitle', 'Выберите документ для изучения') }}</p>
                    </div>
                ...
                <div id="document-{{ $document->id }}" class="document-section mb-12 scroll-mt-24">
                    ...
                    <div class="mb-8 text-center">
                        <div class="inline-flex items-center justify-center px-4 py-2 bg-blue-600/20 border border-blue-500/30 rounded-full mb-4">
                            ...
                            <span class="text-blue-400 text-sm font-semibold">{{ \App\Models\PageSection::getValue('union_education', 'document_header', 'normative_label', 'НОРМАТИВНЫЙ ДОКУМЕНТ') }}</span>
                        </div>
                        ...
                        <div class="flex items-center justify-center space-x-4 text-gray-400 text-sm mb-4">
                            ...
                            @endif
                            <span class="px-3 py-1 bg-blue-600/20 rounded-full text-blue-300">{{ $document->pages }} {{ \App\Models\PageSection::getValue('union_education', 'pdf_viewer', 'pages_label', 'стр.') }}</span>
                        </div>
                    </div>
                        ...
                        <div class="pdf-viewer-container">
                            <div class="pdf-viewer-header flex justify-between items-center mb-4 p-3 bg-gray-900/50 rounded-lg">
                                <div class="flex items-center space-x-4">
                                    <button class="pdf-control-btn zoom-out" title="{{ \App\Models\PageSection::getValue('union_education', 'pdf_viewer', 'zoom_out_title', 'Уменьшить') }}">
                                        <svg class="w-5 h-5 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"/>
                                        </svg>
                                    </button>
                                    <span class="pdf-zoom-level text-blue-300 font-medium">100%</span>
                                    <button class="pdf-control-btn zoom-in" title="{{ \App\Models\PageSection::getValue('union_education', 'pdf_viewer', 'zoom_in_title', 'Увеличить') }}">
                                        <svg class="w-5 h-5 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                                        </svg>
                                    </button>
                                    <span class="text-gray-600 mx-2">|</span>
                                    <span class="pdf-page-info text-gray-400 text-sm">{{ \App\Models\PageSection::getValue('union_education', 'pdf_viewer', 'page_label', 'Страница') }} <span class="font-semibold text-white">1</span> {{ \App\Models\PageSection::getValue('union_education', 'pdf_viewer', 'of_label', 'из') }} <span class="font-semibold text-white">{{ $document->pages }}</span></span>
                                </div>
                                
                                <div class="flex items-center space-x-3">
                                    <a href="{{ route('union-education.preview', ['document' => $document->slug]) }}" 
                                       target="_blank" 
                                       class="pdf-control-btn external-link" 
                                       title="{{ \App\Models\PageSection::getValue('union_education', 'pdf_viewer', 'open_new_tab_title', 'Открыть в новой вкладке') }}">
                                        <svg class="w-5 h-5 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="pdf-viewport rounded-lg overflow-hidden bg-gray-900">
                                <iframe 
                                    src="{{ route('union-education.preview', ['document' => $document->slug]) }}"
                            ...
                            <div class="pdf-navigation flex justify-between items-center mt-4 p-3 bg-gray-900/50 rounded-lg">
                                <button class="pdf-nav-btn prev-page flex items-center text-gray-300 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed">
                                    ...
                                    {{ \App\Models\PageSection::getValue('union_education', 'pdf_viewer', 'prev_button', 'Предыдущая') }}
                                </button>
                                ...
                                <div class="flex items-center space-x-2">
                                    <input type="number" 
                                           ...
                                           max="{{ $document->pages }}" 
                                           value="1">
                                    <span class="text-gray-400">{{ \App\Models\PageSection::getValue('union_education', 'pdf_viewer', 'of_label', 'из') }} {{ $document->pages }}</span>
                                </div>
                                ...
                                <button class="pdf-nav-btn next-page flex items-center text-gray-300 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed">
                                    {{ \App\Models\PageSection::getValue('union_education', 'pdf_viewer', 'next_button', 'Следующая') }}
                    ...
                    <div class="text-center">
                        <a href="{{ route('union-education.download', ['document' => $document->slug]) }}" 
                           ...
                           class="inline-flex items-center px-6 py-3 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors duration-200">
                            ...
                            @if($document->file_size)
                                {{ \App\Models\PageSection::getValue('union_education', 'download', 'download_with_size', 'Скачать документ') }} ({{ $document->file_size }})
                            @else
                                {{ \App\Models\PageSection::getValue('union_education', 'download', 'download_button', 'Скачать документ') }}
                            @endif
                        </a>
                    </div>
                </div>
            ...
            <div class="bg-gradient-to-r from-blue-900/20 to-cyan-900/20 rounded-2xl p-8 border border-blue-500/10">
                <div class="flex flex-col md:flex-row items-center">
                    ...
                    <div class="md:w-3/4 text-center md:text-left">
                        <h3 class="text-2xl font-bold text-white mb-4">{{ \App\Models\PageSection::getValue('union_education', 'info_block', 'title', 'Значение нормативных документов') }}</h3>
                        <p class="text-gray-300 leading-relaxed">
                            {{ \App\Models\PageSection::getValue('union_education', 'info_block', 'description', 'Нормативные документы являются основой для эффективной работы профсоюзных организаций. Они определяют права, обязанности и процедуры деятельности профсоюзных активистов, обеспечивают единый подход к организации профсоюзной работы и способствуют развитию профсоюзного движения. Регулярное изучение и применение этих документов является обязательным для всех членов профсоюзных организаций.') }}
                        </p>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    documentSections.forEach((section, index) => {
        ...
        function updatePagination(currentPage) {
            ...
            pageInfo.innerHTML = `{{ \App\Models\PageSection::getValue('union_education', 'pdf_viewer', 'page_label', 'Страница') }} <span class="font-semibold text-white">${currentPage}</span> {{ \App\Models\PageSection::getValue('union_education', 'pdf_viewer', 'of_label', 'из') }} <span class="font-semibold text-white">${pages}</span>`;
