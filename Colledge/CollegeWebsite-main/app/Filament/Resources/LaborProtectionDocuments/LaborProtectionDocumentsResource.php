...
namespace App\Filament\Resources\LaborProtectionDocuments;

use App\Filament\Resources\LaborProtectionDocuments\Pages;
...
class LaborProtectionDocumentsResource extends Resource
{
    protected static ?string $model = LaborProtectionDocument::class;

    protected static ?string $navigationLabel = 'Охрана труда';
    protected static ?string $modelLabel = 'Документ по охране труда';
    protected static ?string $pluralModelLabel = 'Документы по охране труда';
                        ...
                        ->afterStateUpdated(function (callable $set, $state) {
                            if ($state) {
                                ...
                                if (file_exists($path)) {
                                    ...
                                    try {
                                        $pagesCount = self::getPdfPageCount($path);
    ...
    {
        return [
            'index' => Pages\ListLaborProtectionDocuments::route('/'),
            'create' => Pages\CreateLaborProtectionDocuments::route('/create'),
            'edit' => Pages\EditLaborProtectionDocuments::route('/{record}/edit'),
        ];
    }
    ...
    private static function getPdfPageCount(string $filePath): int
    {
        ...
        if (shell_exec('which pdftk')) {
            $output = shell_exec("pdftk {$filePath} dump_data 2>&1");
        ...
        if (shell_exec('which pdfinfo')) {
            $output = shell_exec("pdfinfo {$filePath} 2>&1");
